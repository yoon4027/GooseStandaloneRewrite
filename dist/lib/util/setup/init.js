...
const init = /* @__PURE__ */ __name(async ({ channel, platform, buildPath }) => {
  const dirPath = join(buildPath, channel, platform);
  const basePath = join(dirPath, platform === "windows" ? "files" : `Discord${channel === "stable" ? "" : toTitleCase(channel)}`);
  ...
  if (!existsSync(tarPath)) {
    switch (platform) {
      ...
      case "windows": {
        ...
        const manifestRequest = await axios.get(manifestUrl);
        ...
        const data = brotliDecompressSync(await axios.get(manifest.full.url, { responseType: "arraybuffer" }));
  ...
  if (platform === "windows") {
    ...
    logger.info("Completed windows specific post-extract update package fixing");
  }
